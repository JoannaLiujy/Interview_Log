...
```
### 4.DNS服务器域名解析
> DNS服务器：域看解析服务器，在服务器上存储着 域名<=>服务器外网iP 的相关记录
...
- 减少直接对DOM的操作（原因是减少DOM的回流和重绘。。。），担待项目基本上都是基于mvvm/mvc数据驱动视图渲染的，对DOM的操作框架本身完成，性能要好很多
